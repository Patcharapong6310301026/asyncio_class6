...
async def write_genre(file_name):
    """
    Uses generator from binaryjazz.us  to write a random genre to the 
    name of the given file
    """
    
    async with aiohttp.ClientSession() as session:
        async with session.get("https://binaryjazz.us/wp-json/genrenator/v1/genre") as respone:
            genre = await respone.json()
            
    async with aiohttp.open(file_name, "w") as new_file:
