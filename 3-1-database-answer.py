...
async def asyncpg_async_get_monitors_many_calls(color="green", id="ALFKI"):
    ...
    rows = await conn.fetch(f"""SELECT (SUM((1 - order_details.discount) * order_details.unit_price * order_details.quantity))::NUMERIC::MONEY AS totalamount FROM orders JOIN order_details ON (orders.order_id=order_details.order_id)
    JOIN customers ON (customers.customer_id=orders.customer_id) WHERE customers.customer_id = '{id}'""")
    for row in rows:
